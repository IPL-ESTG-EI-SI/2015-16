    ...
    class ClientWithProtocolSI
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                byte[] signature = asymmetricAlgorithm.SignData(clearData, 0, clearData.Length, sha);



                Console.Write("Sending  signature... ");
                msg = protocol.Make(ProtocolSICmdType.DIGITAL_SIGNATURE, signature);
                netStream.Write(msg, 0, msg.Length);
                Console.WriteLine("ok");
                Console.WriteLine("   Sent: signature = " + ProtocolSI.ToHexString(signature));
