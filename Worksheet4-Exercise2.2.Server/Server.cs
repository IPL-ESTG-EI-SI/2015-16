    ...
    class ServerWithProtocolSI
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                #region Exchange Signature                 
                // Receive the cipher data
                RSACryptoServiceProvider clientAsymmAlgorithm = new RSACryptoServiceProvider();
                clientAsymmAlgorithm.FromXmlString(clientPublicKey);
                ...
                Console.WriteLine("ok");
                Console.WriteLine("   Received: {0} ", ProtocolSI.ToHexString(signature));

                if(clientAsymmAlgorithm.VerifyData(data, sha, signature))
                {
                    // Answer with a ACK
